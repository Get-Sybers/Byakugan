# STIX 2.1 exchange — the `stix-*` / `cti-*` sub-tools

The engine's exchange interface ([`byakugan/exchange`](../byakugan/exchange)):
detections out as STIX 2.1, the engine's own STIX projection
(`byakugan.stix`, the D4 layer) passed through, OpenCTI as the wire. The
engine stays Elastic; the exchange never re-derives what the projection
already projected. What it emits follows the OASIS *STIX Best Practices
Guide v1.0* (cn01) — the section references below are to it (BP) and to the
STIX 2.1 specification.

```bash
byakugan stix-export --hits detections.jsonl --rules-dir rules/ --out bundle.json
byakugan stix-export --hits alerts.json --bundle stix_bundle.json --case CASE-17 --push
```

In the container the same verbs run env-driven (the multi-tool batch
contract): `BYAKUGAN_STIX_EXPORT_*`, `BYAKUGAN_STIX_BEHAVIOUR_*`,
`BYAKUGAN_CTI_PULL_*`, `BYAKUGAN_CTI_SIGHTINGS_*` — see `byakugan/cli.py`'s
module docstring for each block; the flags below are their pass-through
equivalents.
...
- **Global, content-keyed**: indicator (by detection id), identities and
  ...
  used for them) — and every SCO (§2.9). The namespace seeds are wire-format
  constants frozen in [`objects.py`](../byakugan/exchange/objects.py):
  changing one forks every previously exported object.
...
[`extension/dxdfir-extension.schema.json`](../byakugan/exchange/extension/dxdfir-extension.schema.json):
...
Bump `EXTENSION_VERSION` and `EXTENSION_MODIFIED` in
[`objects.py`](../byakugan/exchange/objects.py) together when the schema
changes; `EXTENSION_MODIFIED` alone when only the definition's metadata moved
(as when the schema/doc locations moved into this repository).
...
(config `stack_version`; the default is the stack the exchange is developed
against — a deployment pins its own).

## Pass-through (`--bundle`, repeatable)

The engine projects its CAR stores (the materialised CAR JSONL + native) to
STIX itself (`byakugan.stix`, `stix_bundle.json` per source) — SCOs,
observed-data, and SROs for both relationship classes, labelled `declared` /
`derived`. A projection bundle is merged object-for-object with ids
untouched; a duplicate id keeps the newest `modified`. The summary counts
relationships per class (the exchange's from the extension, the projection's
from its labels).

## Behaviour sightings (`stix-behaviour`)

The detection lanes joined to the CAR entities they touch: one `sighting` of
the ATT&CK attack-pattern per (detection, entity, technique), anchored to an
`observed-data` keyed on the matched CAR row's **spindle guid**, so every
detector that touches the same entity references the same observation — the
behaviour timeline as the primary axis. `--car` names materialised CAR source
directories (or a tree to walk), `--detections` the detection-lane output
dir, `--case` is required; the join keys and skip accounting live in
[`behaviour.py`](../byakugan/exchange/behaviour.py)'s module docstring.
...
[`opencti.py`](../byakugan/exchange/opencti.py): endpoint and token come from
`BYAKUGAN_OPENCTI_URL` / `BYAKUGAN_OPENCTI_TOKEN` (or `opencti.url` /
`opencti.token` in a `--config` JSON/YAML file that is **not committed**).
There is deliberately no `--token` flag. The transport is an interface
(`post(url, headers, body, timeout)`), so the push is tested with a stub and
no platform — see [`test_stix_export.py`](../tests/test_stix_export.py).

Config precedence: file < environment (`BYAKUGAN_OPENCTI_URL`,
`BYAKUGAN_OPENCTI_TOKEN`, `BYAKUGAN_OPENCTI_CONNECTOR_ID`, `BYAKUGAN_STIX_CASE`,
`BYAKUGAN_STIX_TLP`, `BYAKUGAN_STIX_RULES_DIR`, `BYAKUGAN_STIX_ATTACK_INDEX`,
`BYAKUGAN_STIX_STACK_VERSION`, `BYAKUGAN_STIX_CONTACT`, `BYAKUGAN_STIX_CONFIDENCE`,
`BYAKUGAN_CTI_INDEX`) < flags. See
[`config.py`](../byakugan/exchange/config.py) for the file shape: `contact`
is the producer identity's `contact_information` (BP §3.4), `confidence` an
optional STIX confidence (0..100, BP §4.8) stamped on indicators and
sightings.

## The ATT&CK index

[`data/attack-index.json`](../byakugan/exchange/data/attack-index.json) is a
compact index of MITRE's enterprise-attack STIX bundle — technique external
id -> `attack-pattern` id, name, kill-chain phases, `revoked-by` — so the
export can reference the authoritative objects offline. Regenerate it from a
downloaded bundle:

```bash
python -m byakugan.exchange.attack_index enterprise-attack.json
```

`BYAKUGAN_STIX_ATTACK_INDEX` / config `attack_index` points at another index
or straight at an ATT&CK STIX bundle.

## CTI: OpenCTI indicators in, sightings back out (`cti-pull` / `cti-sightings`)
...
```bash
byakugan cti-pull --out cti.ndjson [--since 2026-08-01T00:00:00Z] [--bundle-out pulled.json]
curl -sS -XPOST "$ES/_bulk" -H 'Content-Type: application/x-ndjson' --data-binary @cti.ndjson
byakugan cti-sightings --alerts alerts.json --case CASE-17 --push
```
...
2. **Copy** — [`cti/indicators.py`](../byakugan/exchange/cti/indicators.py)
   breaks every pattern into its `=` comparisons and lands each value under
   the ECS `threat.indicator.*` field that
   [`cti/pattern-mapping.yml`](../byakugan/exchange/cti/pattern-mapping.yml)
   ...
   [`cti/cti.index-template.json`](../byakugan/exchange/cti/cti.index-template.json)
   describes (`PUT _index_template/cti`; strict mapping — the copy refuses to
...
4. **Sightings back** — [`cti/sightings.py`](../byakugan/exchange/cti/sightings.py)
   turns those alerts into one `sighting` per (indicator, host, matched value)
   whose `sighting_of_ref` is the platform's own indicator id (the indicator
   is not re-emitted), each alert's matched value as the spec's SCO in its own
...
`cti.index` / `BYAKUGAN_CTI_INDEX` / `--index` names the concrete `cti-*`
index the bulk lines target (default `cti-opencti`). Both verbs are exercised
in [`test_cti.py`](../tests/test_cti.py) against recording transports — no
platform, no Elasticsearch, no secrets.

```bash
pytest tests/test_stix_export.py tests/test_cti.py tests/test_stix_behaviour_bridge.py
```
