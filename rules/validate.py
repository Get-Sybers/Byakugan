"""The gate for this repository's Elastic detection rules-as-code (rules/).

The rule files are DATA — one YAML per detection, ES|QL or EQL, each carrying
the contract for the TAGGED EVIDENCE LINE it produces. They live with the
engine: the get-sybers/byakugan image (GoDFIR-toolz) clones this repository
at its pin and bakes ``rules/`` at ``/rules``, where ``stix-export`` resolves
indicator patterns (a runtime mount overrides the baked set). This script is
their validator — run standalone (``python3 rules/validate.py``), by this
repository's pytest suite (``tests/test_rules.py``, which adds the cti-*
template cross-checks), and against the baked tree during the image build,
so a malformed or silently-dropped rule FAILS THE BUILD. It moved here with
the rules from DX_DFIR's retired host-python package, rigor intact.
