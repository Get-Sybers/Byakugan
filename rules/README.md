...
**tagged evidence line** it produces. They live with the engine — the
`get-sybers/byakugan` image (GoDFIR-toolz) clones this repository at its
pin and bakes this directory at `/rules`, where the engine's `stix-export`
resolves indicator patterns; a runtime mount overrides the baked set.
Validated and pinned by [`validate.py`](validate.py) — standalone, as this
repository's own pytest gate (`tests/test_rules.py`, which also holds the
cti-* template cross-checks), and during the image build. They moved here
from DX_DFIR's retired host-python package; their ancestry is the retired
Kusto registry, re-expressed — each rule's `source` block keeps that
provenance.

> **The set is pinned.** `validate.py` carries `PINNED_IDS`: a detection
> can neither be dropped silently nor added unnoticed — the pytest suite and
> the image build fail on any drift, and changing the set means changing the
> pin, reviewed like the rules themselves.

```bash
python3 rules/validate.py             # JSON summary, exit 1 on a bad rule — the same gate the suite and the image build run
```
...
| `language` | yes | `esql` or `eql` — also the STIX indicator's `pattern_type` (a trust-group value beyond `pattern-type-ov`, see [docs/STIX-Exchange.md](../docs/STIX-Exchange.md)) |
...
`validate.py`'s `validate()` fails fast: the first problem raises
...
where are [`byakugan/exchange/cti/`](../byakugan/exchange/cti)'s). Same
...
top-level rule set rather than in it. `validate.py` validates it standalone;
the deep cross-checks — every `threat_mapping` value a field the copy
fills, every `threat_index` pattern one the template covers — run in this
repository's own suite (`tests/test_rules.py` hands
`validate_indicator_match()` the real `byakugan.exchange.cti` template),
now that rule and template share a home.

## Coverage: KQL -> ES|QL / EQL

Every detection the retired registry carried, ported or stub (the suite
and the image build fail if one goes missing):
