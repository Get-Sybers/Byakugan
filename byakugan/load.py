...
network call is stdlib `urllib`.
    ...
    python -m byakugan.load <car-tree> [--out DIR] [--namespace NS] [--force]
...
is ever opened without it, the same opt-in shape as ANAMNESIS_SYMBOLS_ONLINE):
  ...
  push    everything bundle mode does, then POSTs each bundle to
          ...
          component/index templates (model/projection/rendered/) before
...
`byakugan.projection` already decides (SKIP_NO_TIMESTAMP / a raised
...
def project_tree(car_dir: str, sources: list[str], namespace: str) -> dict:
    ...
    byakugan.projection's per-function docstrings)."""
...
def run_setup(es_url: str, headers: dict, context: ssl.SSLContext | None,
    ...
    (model/projection/rendered/), skipping any already applied unchanged;
