...
validate.py/render_elastic.py) so `byakugan.load` — and any other consumer —
can import it without pulling in sqlite3, argparse or the rest of the engine.
`load_contract()` resolves `model/projection/` relative to the package the
way `byakugan/build_data_model.py` resolves `third_party/` (its `_HERE`/
`_ROOT`): a sibling of `byakugan/` under the repo root, not an installed data
...
def known_objects() -> frozenset:
    ...
    `car_<object>.jsonl` basenames `byakugan.load`'s discovery should expect,
