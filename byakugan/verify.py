"""CAR run-through — the correctness gate over a materialised CAR tree.

The promotion gate for CAR correctness, owned by the engine that produces the
model. CAR is MATERIALISED: the pipeline normalises each source into finished
CAR events and writes one ``car_<object>.jsonl`` per object (plus
``car_relationships.jsonl``) under ``<source>/``. This gate asserts what was
actually written: each exercised object has rows, its key fields are POPULATED,
its values are SANE (IPs are IPs, ports are ports, SIDs are SIDs, actions are in
the object's vocabulary), every row TRACES TO ONE ARTEFACT (a non-empty
source_artefact — never data compiled together; source_host is honestly null for
artefacts with no host identity, e.g. Linux utmp / network capture), and the
relationship edges reference real endpoints.

The ``car_action`` vocabulary comes from the engine's OWN model
(:mod:`byakugan.carmodel`, reconstructed from the pinned CAR model) — the object
model never leaves the engine. Extraction faithfulness (a CAR field == its single
source record) is proven in the engine's unit-test suite; this gate is the
end-to-end run-through over the written tree.

Reads an ALREADY-BUILT CAR tree. An object with no rows is reported NOT
EXERCISED. Runnable as ``python -m byakugan.verify [CAR_DIR]`` (default: the
current directory); exits 1 on a failed check, 2 when no CAR is present.
"""
...
def _model_actions():
    """The canonical car_action vocabulary per object — ``{object: {actions}}`` —
    from the engine's OWN model (:mod:`byakugan.carmodel`, reconstructed live from
    the pinned CAR model). Returns None if the model cannot be loaded (the gate
    then reports it, rather than crashing)."""
    try:
...
def run(car_dir: str = ".") -> _Checker:
    ...
    # The car_action vocabulary comes from the engine's own model (pinned CAR).
    actions = _model_actions()
    if actions is None:
        c._fail("CAR action vocabulary — engine model not loadable "
                "(the pinned CAR model is missing from the engine image)")
