...
class OpenCTIClient:
    def __init__(self, url: str | None, token: str | None, *, connector_id: str | None = None,
        ...
        if not url:
            raise ValueError("OpenCTI endpoint is not configured (BYAKUGAN_OPENCTI_URL / opencti.url)")
        if not token:
            raise ValueError("OpenCTI token is not configured (BYAKUGAN_OPENCTI_TOKEN / opencti.token)")
    ...
    def _graphql(self, url: str, headers: Mapping[str, str], body: bytes, *,
        ...
        if status in (401, 403):
            return status, None, "OpenCTI rejected the token (check BYAKUGAN_OPENCTI_TOKEN)"
        try:
