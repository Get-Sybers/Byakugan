"""The exchange verbs (stdlib argparse) — `byakugan`'s STIX/CTI sub-tools pass
argv through here verbatim (`python -m byakugan.exchange` runs it directly).

    byakugan stix-export --hits detections.jsonl [--bundle stix_bundle.json] --out bundle.json [--push]
    byakugan stix-behaviour --car CAR_TREE --detections DIR --case CASE-17 --out bundle.json
    byakugan cti-pull --out cti.ndjson [--since 2026-01-01T00:00:00Z]        # OpenCTI -> cti-* copy
    byakugan cti-sightings --alerts alerts.json --out sightings.json [--push]  # matches -> OpenCTI
...
PROG = "python -m byakugan.exchange"
...
def _cmd_export(args: argparse.Namespace) -> None:
    ...
    if cfg.push and not (cfg.opencti_url and cfg.opencti_token):
        _err("--push needs the OpenCTI endpoint AND token: set $BYAKUGAN_OPENCTI_URL and "
             "$BYAKUGAN_OPENCTI_TOKEN (or opencti.url / opencti.token in --config).")
        raise SystemExit(2)
    try:
...
def _cmd_pull(args: argparse.Namespace) -> None:
    ...
    on the STIX id. Endpoint/token from $BYAKUGAN_OPENCTI_URL / $BYAKUGAN_OPENCTI_TOKEN or the
    ...
    if not args.from_bundle and not (cfg.opencti_url and cfg.opencti_token):
        _err("pull needs the OpenCTI endpoint AND token: set $BYAKUGAN_OPENCTI_URL and "
             "$BYAKUGAN_OPENCTI_TOKEN (or opencti.url / opencti.token in --config) — or give "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "stix-export",
        help="Export detections as STIX 2.1 sightings + indicators, merge projection bundles "
    ...
    p.add_argument("--config", type=Path, metavar="<path>", help="JSON/YAML config file (see docs/STIX-Exchange.md).")
    ...
    p.add_argument("--push", action="store_true",
                   help="Also push to OpenCTI (endpoint/token from $BYAKUGAN_OPENCTI_URL / $BYAKUGAN_OPENCTI_TOKEN "
    ...
    p.add_argument("--index", metavar="<str>",
                   help="The cti-* index the bulk lines target (default: config cti.index / $BYAKUGAN_CTI_INDEX, else cti-opencti).")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if getattr(args, "func", None) is None:     # no verb: help, then exit 2
        parser.print_help()
        raise SystemExit(2)
    args.func(args)
