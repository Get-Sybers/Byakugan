...
# The car_<object>.jsonl schema is the engine's CAR object model. These tuples
# are only the exchange's READ view of that contract — the keys this join
# extracts from each JSONL line, read from the materialised per-object store
# offline. They are not a second definition of the schema; the model is the
...
# ------------------------------------------------------------------- small helpers
# The engine build's own done marker: pipeline.py ALWAYS writes it per source,
# even an empty one, so it is the reliable signal that a directory is a
# finished CAR source — independent of which objects it populated (downstream
# orchestration keys on the same file).
