...
2. **Projection bundles pass through.** The engine projects its CAR stores to
   STIX itself (:mod:`byakugan.stix`: SCOs / observed-data / SROs derived from
   the materialised CAR JSONL + native, both relationship classes labelled
   ``declared`` / ``derived``). The exchange never re-derives that: a
   projection bundle is merged object-for-object, ids untouched, into the
   same output bundle (:mod:`.export`).
...
the platform's indicators. The engine is Elastic's; OpenCTI is the wire.
    ...
    byakugan stix-export --hits detections.jsonl --bundle byakugan.json --out bundle.json [--push]
    byakugan cti-pull --out cti.ndjson
    byakugan cti-sightings --alerts alerts.json --out sightings.json [--push]
"""
