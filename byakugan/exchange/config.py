...
the environment (``BYAKUGAN_OPENCTI_URL`` / ``BYAKUGAN_OPENCTI_TOKEN``) or from an
...
Config file shape (every key optional)::
    ...
    opencti:
      ...
      token: ...                 # prefer BYAKUGAN_OPENCTI_TOKEN
      connector_id: ...          # optional; a deterministic default otherwise
    cti:
      index: cti-opencti         # the cti-* index `byakugan cti-pull` writes for
"""
...
ENV_URL = "BYAKUGAN_OPENCTI_URL"
ENV_TOKEN = "BYAKUGAN_OPENCTI_TOKEN"
ENV_CONNECTOR_ID = "BYAKUGAN_OPENCTI_CONNECTOR_ID"
ENV_CASE = "BYAKUGAN_STIX_CASE"
ENV_TLP = "BYAKUGAN_STIX_TLP"
ENV_RULES_DIR = "BYAKUGAN_STIX_RULES_DIR"
ENV_ATTACK_INDEX = "BYAKUGAN_STIX_ATTACK_INDEX"
ENV_STACK_VERSION = "BYAKUGAN_STIX_STACK_VERSION"
ENV_CONTACT = "BYAKUGAN_STIX_CONTACT"
ENV_CONFIDENCE = "BYAKUGAN_STIX_CONFIDENCE"
ENV_CTI_INDEX = "BYAKUGAN_CTI_INDEX"
DEFAULT_CTI_INDEX = "cti-opencti"
# The Elastic stack the exchange is developed against: the version an
# ES|QL / EQL / KQL pattern is known to run on — the indicator's
# pattern_version (STIX 2.1 §4.7). A deployment pins its own via config/env
# (DX_DFIR holds its inventory pin equal to this default in its own tests).
...
class StixConfig:
    ...
    out: str | None = None
    rules_dir: str | None = None            # None: no rules — required to export hits
