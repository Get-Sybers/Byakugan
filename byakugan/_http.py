...
`byakugan.load`, Kibana): auth headers, a TLS context, and the one JSON
request primitive every ES/Kibana call in this package goes through.

Split out of `byakugan.load` (epic #99 phase 5, `byakugan timeline
...
building an Authorization header or a CA-verified context) — `byakugan.load`
re-exports these three names unchanged (`from . import _http` then
`load.auth_headers is _http.auth_headers`, etc.), so nothing that already
imports them off `byakugan.load` needs to change.
