"""The ECS->CAR inverse projector: `byakugan.projection`'s companion, used
only by `byakugan timeline --elastic` (epic #99 phase 5).

`byakugan.projection` turns a CAR row into an Elasticsearch document. This
module undoes exactly that move — a `logs-car.<object>-*` / `logs-car.rel-*`
...
Mechanically derived, every time, from `byakugan.projection.load_contract()`
...
the module docstring of `byakugan.projection` documents.

--------------------------------------------------------------------------- #
Round-trip fidelity, and what stays genuinely irrecoverable
--------------------------------------------------------------------------- #

Two forward-pass gaps (byakugan/projection.py `_apply_object_fields` /
...
from . import carmodel, projection
...
# --------------------------------------------------------------------------- #
# The common header, inverted (mirrors byakugan/projection.py _apply_header).
# --------------------------------------------------------------------------- #
...
def _invert_group(doc: dict, name: str, target: str, group: dict) -> dict:
    ...
    ALWAYS its car.<object>.<field> capture (byakugan/projection.py's fix:
...
def _invert_object_fields(doc: dict, name: str, plan: dict) -> dict:
    ...
    native: true field, and (byakugan/projection.py's second fix) the
