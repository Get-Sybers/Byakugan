...
def push_stream(es_url: str, stream: str, docs: list[tuple[str, dict]], headers: dict,
    ...
    for chunk in _chunk_docs(docs, BULK_MAX_ACTIONS, BULK_MAX_BYTES):
        # refresh=true: a bulk write is not SEARCHABLE until a refresh, and the
        # ids-query verification runs immediately after the push — without this
        # it undercounts and books the shortfall as failed (the live e2e gate
        # caught exactly that). Refreshing via the write request itself, not a
        # separate POST _refresh, keeps the loader inside the least-privilege
        # writer role (a standalone refresh needs the `maintenance` privilege
        # logs_car_writer deliberately does not have).
        url = (f"{es_url}/{stream}/_bulk?refresh=true"
