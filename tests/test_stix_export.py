...
from byakugan.exchange import cli as stix_cli
from byakugan.exchange import attack_index, config, export, hits, objects, opencti
...
def test_pattern_types_are_documented_trust_group_values():
    # BP §8.1: the Kibana languages are trust-group values beyond the spec's
    # pattern-type-ov, named and documented — never mixed into the spec's list.
    # (Which stack version a DEPLOYMENT pins is that deployment's own gate.)
    ...
    readme = (REPO / "docs/STIX-Exchange.md").read_text()
...
def test_extension_schema_covers_everything_emitted(rules):
    schema = json.loads((REPO / "byakugan/exchange/extension/dxdfir-extension.schema.json").read_text())
...
def test_config_layers_file_env_and_overrides(tmp_path):
    ...
    env = {"BYAKUGAN_OPENCTI_URL": "https://env.test", "BYAKUGAN_OPENCTI_TOKEN": "env-token",
           "BYAKUGAN_STIX_ATTACK_INDEX": "/tmp/attack.json"}
    ...
    assert config.load_config(env={"BYAKUGAN_STIX_CONFIDENCE": "0"}).confidence == 0
    assert config.load_config(env={"BYAKUGAN_STIX_CONTACT": " "}).contact is None
...
def test_cli_stix_export(tmp_path, monkeypatch, capsys, rules):
    jsonl = tmp_path / "hits.jsonl"
    jsonl.write_text(json.dumps(ENVELOPE) + "\n")
    out = tmp_path / "bundle.json"
    rules_argv = ["--rules-dir", rules.rules_dir]     # no default: the engine ships no rules
    code, stdout, stderr = _stix(["stix-export", "--hits", str(jsonl), "--out", str(out),
                                  "--case", "CASE-7", "--tlp", "green", *rules_argv], capsys)
    ...
    assert _one(bundle, "indicator")["pattern_version"] == config.DEFAULT_STACK_VERSION
    ...
    monkeypatch.setenv("BYAKUGAN_OPENCTI_URL", "https://env.test")
    monkeypatch.setenv("BYAKUGAN_OPENCTI_TOKEN", "env-token")
    code, stdout, stderr = _stix(["stix-export", "--hits", str(jsonl), "--out", str(out), "--push",
                                  *rules_argv], capsys)
    assert code == 0, stdout + stderr
    ...
    assert _stix(["stix-export", "--hits", str(jsonl), "--push", *rules_argv], capsys)[0] == 1
    monkeypatch.delenv("BYAKUGAN_OPENCTI_TOKEN")
    assert _stix(["stix-export", "--hits", str(jsonl), "--push", *rules_argv], capsys)[0] == 2
    assert _stix(["stix-export"], capsys)[0] == 2
