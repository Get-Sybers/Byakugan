...
from byakugan.exchange import config, export, objects, opencti
from byakugan.exchange.cti import indicators as ind
from byakugan.exchange.cti import sightings as sg
from byakugan.exchange.hits import flatten
...
def test_config_cti_index_layers(tmp_path):
    ...
    assert config.load_config(str(f), env={"BYAKUGAN_CTI_INDEX": "cti-env"}).cti_index == "cti-env"
    assert config.load_config(str(f), env={"BYAKUGAN_CTI_INDEX": "cti-env"}, cti_index="cti-flag").cti_index == "cti-flag"
...
def test_cli_stix_pull_and_sightings(tmp_path, monkeypatch, capsys):
    for var in ("BYAKUGAN_OPENCTI_URL", "BYAKUGAN_OPENCTI_TOKEN", "BYAKUGAN_CTI_INDEX"):
        monkeypatch.delenv(var, raising=False)
    bundle_path = tmp_path / "indicators.json"
    bundle_path.write_text(json.dumps({"type": "bundle", "id": "bundle--" + str(uuid.uuid4()), "objects": BUNDLE_OBJECTS}))
    out = tmp_path / "cti.ndjson"
    code, stdout, stderr = _stix(["cti-pull", "--from-bundle", str(bundle_path), "--out", str(out), "--index", "cti-case7"],
                                 capsys)
    ...
    code, stdout, stderr = _stix(["cti-pull", "--from-bundle", str(bundle_path)], capsys)
    assert code == 0, stdout + stderr
    assert json.loads(stdout.splitlines()[0]) == {"index": {"_index": "cti-opencti", "_id": IND_IP}}
    assert _stix(["cti-pull"], capsys)[0] == 2
    ...
    monkeypatch.setenv("BYAKUGAN_CTI_INDEX", "cti-env")
    code, stdout, stderr = _stix(["cti-pull", "--out", str(out), "--since", "2026-08-01T00:00:00Z"], capsys)
    assert code == 0, stdout + stderr
    ...
    assert _stix(["cti-pull"], capsys)[0] == 1
    assert _stix(["cti-pull", "--since", "yesterday"], capsys)[0] == 2
    ...
    code, stdout, stderr = _stix(["cti-sightings", "--alerts", str(alerts), "--out", str(sightings_out),
    ...
    assert _stix(["cti-sightings"], capsys)[0] == 2
    plain = tmp_path / "plain.jsonl"
    plain.write_text(json.dumps({"event": {"id": "x"}}) + "\n")
    assert _stix(["cti-sightings", "--alerts", str(plain)], capsys)[0] == 2
