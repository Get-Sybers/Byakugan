...
def test_http_limits_hostname_and_url_reconstruction():
    ...
    t = go_normalize("zeek_http", dict(rec, method="CONNECT"))
...
def test_zeek_http_get_and_iso_or_epoch_ts():
    ...
    ev2 = go_normalize("zeek_http", dict(rec, ts=1341856306.5))
    assert ev2["timestamp"].startswith("2012-07-09T")          # epoch converts
    assert go_normalize("zeek_http", dict(rec, method="HEAD")) is None
