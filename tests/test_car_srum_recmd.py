...
def test_srum_network_usage_is_flow_message_with_attribution():
    ev = go_normalize("l2t_srum", _srum(
    ...
    dev = go_normalize("l2t_srum", _srum(
...
def test_srum_application_usage_is_execution_evidence():
    ...
    assert go_normalize("l2t_srum", _srum(
...
def test_recmd_value_record_is_registry_value_edit():
    ev = go_normalize("recmd_batch", _recmd())
    ...
    assert go_normalize("recmd_batch", _recmd(Deleted=True)) is None
