...
import json
with open("go/internal/ir/ir.json", encoding="utf-8") as _fh:
    d = json.load(_fh)
...
lines.append("// source, now authored in Go. Regenerate with scripts/gen_ir_sections.py (bootstrap");
...
for s in sections:
    ...
    lines.append("")
_body = "\n".join(lines)
with open("go/internal/authoring/ir_sections.go", "w", encoding="utf-8") as _out:
    _out.write(_body)
print("wrote go/internal/authoring/ir_sections.go", len(_body), "bytes")
